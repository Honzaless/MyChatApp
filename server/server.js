  ...
  if (foundUser) {
    ...
    if (isPass) {
      jwt.sign({userId: foundUser._id, username}, jwtSecretKey, {}, (err, token) => {
        res.cookie("token", token, {sameSite: "strict", secure: true, maxAge: 36000000, domain: "mychatapp-frontend.onrender.com", path: "/", httpOnly: true}).json({
          id: foundUser._id,
        })
      });
    }
...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    jwt.sign({userId: createdUser._id, username}, jwtSecretKey, {}, (err, token) => {
      if (err) throw err;
      res.cookie("token", token, {sameSite: "strict", secure: true, maxAge: 36000000, domain: "mychatapp-frontend.onrender.com", path: "/", httpOnly: true}).status(201).json({
        id: createdUser._id,
      });
  });
  }
...
app.post("/logout", (req, res) => {
  res.clearCookie("token", {sameSite: "strict", secure: true, domain: "mychatapp-frontend.onrender.com", path: "/", httpOnly: true});
  res.end();
});
